...
global $_W,$_GPC;
		...
		else
		{
			...
			{
				echo "<script>
					window.location.href = '".$this->createMobileUrl('userinfo',array('op'=>'visit'))."';
				</script>";
			}
			else
			{
		...
		if(!empty($member))
		{
			...
			if($op=='more')
			{
				...
				$last=pdo_fetch("SELECT visitid,createtime FROM ".tablename('jy_ppp_visit')." WHERE weid=".$weid." AND id=".$lastid);
				if(empty($last['createtime']))
				{
					echo json_encode(array('status'=>4));
					exit;
				}
				$visit=pdo_fetchall("SELECT a.id,a.visitid as mid,b.avatar,b.nicheng,b.province,b.brith,b.sex,c.height FROM ".tablename('jy_ppp_visit')." as a left join ".tablename('jy_ppp_member')." as b on a.visitid=b.id left join ".tablename('jy_ppp_basic')." as c on a.visitid=c.mid WHERE a.weid=".$weid." AND a.mid=".$mid." AND a.createtime<".$last['createtime']." GROUP BY a.visitid ORDER BY a.createtime DESC LIMIT 10 ");
				$html='';
				if(!empty($visit))
				{
			...
			else
			{
				...
				$visit=pdo_fetchall("SELECT a.id,a.visitid as mid,b.avatar,b.nicheng,b.province,b.brith,b.sex,c.height FROM ".tablename('jy_ppp_visit')." as a left join ".tablename('jy_ppp_member')." as b on a.visitid=b.id left join ".tablename('jy_ppp_basic')." as c on a.visitid=c.mid WHERE a.weid=".$weid." AND a.mid=".$mid." GROUP BY a.visitid ORDER BY a.createtime DESC LIMIT 10 ");
