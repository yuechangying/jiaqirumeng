...
global $_W,$_GPC;
		...
		else
		{
			...
			{
				echo "<script>
					window.location.href = '".$this->createMobileUrl('userinfo',array('op'=>'loveme'))."';
				</script>";
			}
			else
			{
		...
		if(!empty($member))
		{
			...
			if($op=='more')
			{
				...
				if(!empty($love))
				{
					foreach ($love as $key => $l) {
						...
						$temp=pdo_fetch("SELECT createtime FROM ".tablename('jy_ppp_xinxi')." WHERE weid=".$weid." AND sendid=".$mid." AND mid=".$l['attentid']." AND type=0 ORDER BY createtime DESC LIMIT 1 ");
						...
						{
							$a_zh='<span class="hello" onclick="sayPeople2(this,'.$l['attentid'].')" >打招呼</span>';
						}
						$html.='<ul class="disbox-hor user-list" data-id="'.$l['id'].'"><li class="foot-icon disbox-center"><a href="'.$this->createMobileUrl('detail',array('id'=>$l['attentid'])).'"><img src="'.$avatar.'"</a></li><li class="disbox-flex user_mession"><a href="'.$this->createMobileUrl('detail',array('id'=>$l['attentid'])).'"><b class="tit">'.$l['nicheng'].'</b><p class="bot">'.$ziliao.'</p></a></li><div class="right">'.$a_zh.'</div></ul>';
					}
				}
				if(count($love)<10)
				{
					return json_encode(array('status'=>2,'log'=>$html));
					exit;
				}
				else
				{
			...
			else
			{
				...
				$today=strtotime("today");
				$love=pdo_fetchall("SELECT a.id,a.attentid,a.mid,b.avatar,b.nicheng,b.province,b.brith,b.sex,c.height FROM ".tablename('jy_ppp_attent')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('jy_ppp_basic')." as c on a.mid=c.mid WHERE a.weid=".$weid." AND a.attentid=".$mid." ORDER BY a.createtime DESC LIMIT 10 ");
				if(!empty($love))
				{
					foreach ($love as $key => $value) {
						$temp=pdo_fetch("SELECT createtime FROM ".tablename('jy_ppp_xinxi')." WHERE weid=".$weid." AND sendid=".$mid." AND mid=".$value['mid']." AND type=0 ORDER BY createtime DESC LIMIT 1 ");
						if($temp['createtime']>$today)
						{
							$love[$key]['zh']=1;
						}
