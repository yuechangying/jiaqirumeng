...
global $_W,$_GPC;
		...
		else
		{
			...
			{
				echo "<script>
					window.location.href = '".$this->createMobileUrl('userinfo',array('op'=>'myblack'))."';
				</script>";
			}
			else
			{
		...
		if(!empty($member))
		{
			...
			{
				$id=$_GPC['id'];
				pdo_delete('jy_ppp_black',array('id'=>$id));
				echo 1;
				exit;
			}
			else
			{
				...
				$black=pdo_fetchall("SELECT a.id,a.blackid,a.mid,b.avatar,b.nicheng,b.province,b.brith,b.sex,c.height FROM ".tablename('jy_ppp_black')." as a left join ".tablename('jy_ppp_member')." as b on a.blackid=b.id left join ".tablename('jy_ppp_basic')." as c on a.blackid=c.mid WHERE a.weid=".$weid." AND a.mid=".$mid." ORDER BY a.createtime DESC ");
				include $this->template('myblack');
			}
