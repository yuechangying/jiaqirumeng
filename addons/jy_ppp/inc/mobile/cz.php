...
global $_W,$_GPC;
		...
		else
		{
			...
			{
				echo "<script>
					window.location.href = '".$this->createMobileUrl('userinfo',array('op'=>'cz'))."';
				</script>";
			}
			else
			{
		...
		if(!empty($member))
		{
			$category2 = pdo_fetchall ( "SELECT * FROM " . tablename ( 'jy_ppp_price' ) . " WHERE weid = ".$weid." AND log=1 AND enabled=1 ORDER BY displayorder DESC,id ASC" );
			$category = pdo_fetchall ( "SELECT * FROM " . tablename ( 'jy_ppp_price' ) . " WHERE weid = ".$weid." AND log=2 AND enabled=1 ORDER BY displayorder DESC,id ASC" );
