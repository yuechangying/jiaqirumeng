...
global $_W,$_GPC;
		...
		else
		{
			...
			{
				echo "<script>
					window.location.href = '".$this->createMobileUrl('userinfo',array('op'=>'shouxin'))."';
				</script>";
			}
			else
			{
		...
		if(!empty($member))
		{
			$category = pdo_fetchall ( "SELECT * FROM " . tablename ( 'jy_ppp_price' ) . " WHERE weid = ".$weid." AND log=3 AND enabled=1 ORDER BY displayorder DESC,id ASC" );
