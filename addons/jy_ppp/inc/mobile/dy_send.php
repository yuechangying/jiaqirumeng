...
global $_W,$_GPC;
		...
		else
		{
			...
			{
				echo "<script>
					window.location.href = '".$this->createMobileUrl('userinfo',array('op'=>'dy_send','id'=>$yhid))."';
				</script>";
			}
			else
			{
		...
		$yh=pdo_fetch("SELECT * FROM ".tablename('jy_ppp_member')." WHERE weid=".$weid." AND id=".$yhid);
		if(empty($yh))
		{
			message("操作非法！请返回重试!！");
		}
