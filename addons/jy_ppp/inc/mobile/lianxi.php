...
global $_W,$_GPC;
		...
		if(!empty($member))
		{
			...
			{
                printLog(basename(__FILE__) . ",line=" . __LINE__ . ", YYY mid=" . $mid);

				$sitem=pdo_fetch("SELECT * FROM ".tablename('jy_ppp_setting')." WHERE weid=".$weid);
				$lianxi=pdo_fetchall("SELECT a.id,a.mid as mid,b.avatar,b.nicheng,b.province,b.brith,b.sex,b.type,c.height FROM ".tablename('jy_ppp_xinxi')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('jy_ppp_basic')." as c on a.mid=c.mid WHERE a.weid=".$weid." AND a.sendid=".$mid." AND a.type!=0 GROUP BY a.mid ORDER BY a.createtime DESC LIMIT 10 ");
