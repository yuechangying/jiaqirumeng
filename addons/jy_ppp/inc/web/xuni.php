...
global $_W,$_GPC;
		...
		if(checksubmit()) {
			...
			$birth_max=strtotime($_GPC['birth1']);
			$birth_min=strtotime($_GPC['birth2']);
			if($birth_max<$birth_min)
			{
				message("出生日期最小值不得大于最大值!");
			}
			$height1=intval($_GPC['height1']);
			$height2=intval($_GPC['height2']);
			if($height1>$height2)
			{
				message("身高最小值不得大于最大值!");
			}
			$weight1=intval($_GPC['weight1']);
			$weight2=intval($_GPC['weight2']);
			if($weight1>$weight2)
			{
				message("体重最小值不得大于最大值!");
			}
...
��","那过期的诺言即使破碎，我也会把他埋葬在心里","掩埋了泪眼的斑驳，在转角之后扣上故事门锁","你于我如阳光，但总有日落的时候","在看不见的角落里，阳光照不进的秘密","瞧那路灯，暗黄色，照亮了那一隅之地，牵扯出缠绵的过往。","若相惜 何须执手问年华 ","浮云如你，又怎能为平凡停留","谁为谁画地为牢。谁又为谁桑海仓田。","缘分，默默地守候在某个渡口，等待着爱情的携带。两颗空洞的心，不约而同。","我的淚，落在残花上，摔的粉碎。","那就由我陪你, 看遍所有会笑的星空。","渐渐的我学会了隐身，渐渐的我不喜欢在群里说话，渐渐的我不喜欢和别人聊天，渐渐的身边的人越来越少。","我不能让所有人都满意，因为不是所有人都是人！","谱写自己的路，别再让身影失去平衡而慢慢下沉。",
			...
			if($province){
				$thumb_data=pdo_fetchall("select avatar FROM ".tablename('jy_ppp_member')."  where weid=".$weid." AND province='".$province."'");
				foreach($thumb_data as $vo){
					$thumb_arr[]="'".$vo['avatar']."'";
				}
				if($thumb_arr){
					$thumb_where=" and thumb not in(".implode(',',$thumb_arr).")";
				}
				$xuni_num=pdo_fetchall("SELECT mid from ".tablename('jy_ppp_xunithumb')." WHERE sex=".$sex.$thumb_where);
				echo mysql_error();
			}
			if(empty($xuni_num)){
			...
			for ($i=0; $i < $num; $i++) {
				...
				$brith=mt_rand($birth_min,$birth_max);
				...
				if(!empty($xuni_num))
				{
					...
					$thumb=pdo_fetchall("SELECT * FROM ".tablename('jy_ppp_xunithumb')." WHERE mid=".$thumb_num.$thumb_where);
					$avatar_l=pdo_fetch("SELECT * FROM ".tablename('jy_ppp_xunithumb')." WHERE mid=".$thumb_num." AND avatar=1 ".$thumb_where);
				...
				//$cun=pdo_fetch("SELECT * FROM ".tablename('jy_ppp_member')." WHERE (nicheng='".$nicheng."' or avatar='".$avatar_l."' ) and (province=".$province.' and city='.$city.')');
				$cun=pdo_fetch("SELECT * FROM ".tablename('jy_ppp_member')." WHERE (nicheng='".$nicheng."' or avatar='".$avatar_l."' ) AND weid=".$weid);
				if($cun){
					continue;
				}
				...
				$height=mt_rand($height1,$height2);

				$blank=0;
				
				//$weight=mt_rand($weight1,$weight2);
				$car_attr=$_GPC['car'];
				if(!empty($car_arr))
				{
					$car=array_rand($car_arr);
					$car=$car_arr[$car];
				}
				if(empty($car))
				{
				...
				$constellation_arr=$_GPC['constellation'];
				if(!empty($constellation_arr))
				{
					$constellation=array_rand($constellation_arr);
					$constellation=$constellation_arr[$constellation];
				}
				...
				$blank=0;
				$lifestatus_arr=$_GPC['lifestatus'];
				if(!empty($lifestatus_arr))
				{
					$lifestatus=array_rand($lifestatus_arr);
					$lifestatus=$lifestatus_arr[$lifestatus];
				}
				if(empty($lifestatus))
				{
					$lifestatus=0;
				...
				$jobstatus_arr=$_GPC['jobstatus'];
				if(!empty($jobstatus_arr))
				{
					$jobstatus=array_rand($jobstatus_arr);
					$jobstatus=$jobstatus_arr[$jobstatus];
				}
				if(empty($jobstatus))
				{
					$jobstatus=0;
				...
				$companytype_arr=$_GPC['companytype'];
				if(!empty($companytype_arr))
				{
					$companytype=array_rand($companytype_arr);
					$companytype=$companytype_arr[$companytype];
				}
				...
				$smoke_arr=$_GPC['smoke'];
				if(!empty($smoke_arr))
				{
					$smoke=array_rand($smoke_arr);
					$smoke=$smoke_arr[$smoke];
				}
				if(empty($smoke))
				{
					$smoke=0;
				...
				$parentstatus_arr=$_GPC['parentstatus'];
				if(!empty($parentstatus_arr))
				{
					$parentstatus=array_rand($parentstatus_arr);
					$parentstatus=$parentstatus_arr[$parentstatus];
				}
				...
				$chuyi_arr=$_GPC['chuyi'];
				if(!empty($chuyi_arr))
				{
					$chuyi=array_rand($chuyi_arr);
					$chuyi=$meili_arr[$chuyi];
				}
				if(empty($chuyi))
				{
					$chuyi=0;
			...
			message("设置成功!新增".$add_num.'人',$this->createWebUrl('xuni'),'success');
		}
