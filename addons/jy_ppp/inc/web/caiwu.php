...
global $_W,$_GPC;
		...
		if($op=='pay')
		{
			$pay_list=pdo_fetchall("SELECT a.createtime,a.fee,a.id,a.paytime,a.log,b.nicheng,b.mobile as mobile2,b.mobile_auth,c.nickname,c.avatar,d.rulename FROM ".tablename('jy_ppp_pay_log')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('mc_members')." as c on b.uid=c.uid left join ".tablename('jy_ppp_rule')." as d on b.qrcode_id=d.ruleid WHERE a.weid= ".$weid." AND a.status=1 AND a.fee>0 ".$condition2." ORDER BY b.qrcode_id DESC LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
			$pay_total = count(pdo_fetchall("SELECT a.id FROM ".tablename('jy_ppp_pay_log')." as a  WHERE a.weid=".$weid." AND a.fee>0 AND a.status=1 ".$condition2));
			$pager = pagination($pay_total, $pindex, $psize);
		}
		if($op=='credit')
		{
			$credit_list=pdo_fetchall("SELECT a.createtime,a.fee,a.id,a.paytime,a.log,b.nicheng,b.mobile as mobile2,b.mobile_auth,c.nickname,c.avatar,d.rulename FROM ".tablename('jy_ppp_pay_log')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('mc_members')." as c on b.uid=c.uid left join ".tablename('jy_ppp_rule')." as d on b.qrcode_id=d.ruleid WHERE a.weid= ".$weid." AND a.status=1 AND a.log=1 AND a.fee>0".$condition2." ORDER BY b.qrcode_id DESC LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
			$credit_total = count(pdo_fetchall("SELECT a.id FROM ".tablename('jy_ppp_pay_log')." as a  WHERE a.weid=".$weid." AND a.fee>0 AND a.status=1 AND a.log=1 ".$condition2));
			$pager = pagination($credit_total, $pindex, $psize);
		}
		if($op=='baoyue')
		{
			$baoyue_list=pdo_fetchall("SELECT a.createtime,a.fee,a.id,a.paytime,a.log,b.nicheng,b.mobile as mobile2,b.mobile_auth,c.nickname,c.avatar,d.rulename FROM ".tablename('jy_ppp_pay_log')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('mc_members')." as c on b.uid=c.uid left join ".tablename('jy_ppp_rule')." as d on b.qrcode_id=d.ruleid WHERE a.weid= ".$weid." AND a.status=1 AND a.log=2 AND a.fee>0".$condition2." ORDER BY b.qrcode_id DESC LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
			$baoyue_total = count(pdo_fetchall("SELECT a.id FROM ".tablename('jy_ppp_pay_log')." as a  WHERE a.weid=".$weid." AND a.fee>0 AND a.status=1 AND a.log=2 ".$condition2));
			$pager = pagination($baoyue_total, $pindex, $psize);
		}
		if($op=='shouxin')
		{
			$shouxin_list=pdo_fetchall("SELECT a.createtime,a.fee,a.id,a.paytime,a.log,b.nicheng,b.mobile as mobile2,b.mobile_auth,c.nickname,c.avatar,d.rulename FROM ".tablename('jy_ppp_pay_log')." as a left join ".tablename('jy_ppp_member')." as b on a.mid=b.id left join ".tablename('mc_members')." as c on b.uid=c.uid left join ".tablename('jy_ppp_rule')." as d on b.qrcode_id=d.ruleid WHERE a.weid= ".$weid." AND a.status=1 AND a.log=3 AND a.fee>0".$condition2." ORDER BY b.qrcode_id DESC LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
			$shouxin_total = count(pdo_fetchall("SELECT a.id FROM ".tablename('jy_ppp_pay_log')." as a  WHERE a.weid=".$weid." AND a.fee>0 AND a.status=1 AND a.log=3 ".$condition2));
